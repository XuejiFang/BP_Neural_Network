...
while epoch_num<Epochs_max
    ...
    y_pre_std_y1=logsig(y_pre_std_y);
...
%%梯度下降
    ...
    c_wj=2*(y_pre_std_y1-y_train_std).*y_pre_std_y1.*(1-y_pre_std_y1)*y_pre_std_u1';
    c_theta_y=2*(y_pre_std_y1-y_train_std).*y_pre_std_y1.*(1-y_pre_std_y1)*ones(train_num,1);
    c_vij=wj'*2*(y_pre_std_y1-y_train_std).*y_pre_std_y1.*(1-y_pre_std_y1).*(y_pre_std_u1).*(1-y_pre_std_u1)*x_train_std';
    c_theta_u=wj'*2*(y_pre_std_y1-y_train_std).*y_pre_std_y1.*(1-y_pre_std_y1).*(y_pre_std_u1).*(1-y_pre_std_u1)*ones(train_num,1);
...
test_out=logsig(wj*test_put+repmat(theta_y,1,test_num));
